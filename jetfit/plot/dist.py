...
class DistributionPlot:
    ...
    def spectral_index(self, indices, out_dir=None):
        """
        ...
        for index in indices:
            ...
            # Calculate the most likely spectral index
            best_samp = self.get_best_params(
                as_dict=True, group=self.get_group(time)
            )
            best_modeled = model_spectral_index(
                index, best_samp['model']
            )

            # SpectralIndexModel returns an array always. Will
            # change this in the future. For now, just take the
            # first since I always want a single value here.
            if isinstance(best_modeled, np.ndarray):
                if best_modeled.size != 1:
                    raise ValueError(
                        f'Received unexpected spectral index: '
                        f'{best_modeled}'
                    )
                best_modeled = best_modeled[0]

            # Create the various AMAZING plots
            title = f"Spectral Index Distribution"
            if self.get_group(time) is not None:
                title += f" (DataGroup='{self.get_group(time)}')"

            self.plot_spectral_index(
                modeled, round(best_modeled, 3), index, title, out_dir
            )

    @staticmethod
    def plot_spectral_index(dist, best, index, title=None, out_dir=None):
        """
        ...
        index : SpectralIndex
            The index that the distribution was modeled after.
        ...
        # Plot the XRT value
        l = index.uncertainty.lower.value
        u = index.uncertainty.upper.value

        plt.axvline(  # Plot the position of the value
            index.value.value, color='tab:purple', linestyle='--',
            linewidth=2, label=f'XRT Value: {index.value.value} (+{u}, -{l})'
        )
        plt.axvspan(  # Plot the error region
            index.value.value - l, index.value.value +u, color='tab:purple', alpha=0.2
        )
        ...
        for ct, l, r in zip(cts, bins[:-1], bins[1:]):
            if int(ct) != 0:
                plt.text(
                    (l + r) / 2, ct + 0.1, str(int(ct)), ha='center', va='bottom')
    ...
    def beaming(self, out_dir=None):
        ...
        best_samp = self.get_best_params(
            as_dict=True, group=self.get_group(tj)
        )['model']
        ...
        if self.get_group(tj) is not None:
            title += f" (DataGroup='{self.get_group(tj)}')"
