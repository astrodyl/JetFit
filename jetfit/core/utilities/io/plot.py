...
class Plot:
    ...
    @staticmethod
    def plot_lc(df, params: dict, path: str):
        """ """

        # Plot XRT data
        _, ax = plt.subplots(figsize=(8, 8))
        ax.errorbar(df['Times'], df['Fluxes'], yerr=df['FluxErrs'], color='blue', fmt='.', label='XRT')
        ax.set_yscale('log'), ax.set_xscale('log')
        ax.set_ylabel('0.3 keV - 10 keV (erg / cm^2 / s)')
        ax.set_xlabel('Time (s)')
        ax.legend()

        flux_generator = FluxGenerator(path, log_axis=['tau'])
        new_times = np.linspace(df['Times'].min() * 1.0, df['Times'].max() * 2.0, 200)

        f_peak, nu_c, nu_m = flux_generator.get_transformed_value(
            flux_generator.get_taus(new_times, params), params
        )

        # Plot flux model
        flux_model = []
        for j in range(len(f_peak)):
            flux_model.append(flux_generator.get_integrated(
        ...
        plt.loglog(new_times, np.array(flux_model), '--', color='black', linewidth=1.5)
        plt.show()
